...
int main(int argc, char* argv[])
{
	...
	{
		sleep(period);
		std::cout<<"[2] - Подготовка к входу в сегмент"<<std::endl;
		lock(share,SECOND);//получение доступа
		...
		{//сигнал окончания работы
			++(share->stop);
			unlock(share,SECOND);//выкл заинтересованность в ресурсе
			std::cout<<"[2] - Выход из сегмента"<<std::endl;
